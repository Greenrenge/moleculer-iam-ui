...
export const AppStateContext = createContext<
  [ApplicationState, AppStateProvider["dispatch"]]
>([] as any)
    ...
    return fetch(url, {
      ...
      (res) => {
        ...
        return res.json().then(
          (data: ApplicationResponse): ApplicationState => {
            if (data.error) {
              // got error response
              ...
              if (res.status === 422 && data.error.data) {
                ...
                const fields = data.error.data.reduce((obj, entry) => {
                  if (!obj[entry.field]) {
                    ...
                    obj[entry.field] = f(
                      {
                        id: `error.${data.error!.error}.data.${entry.type}`,
                      ...
                      },
                      {
                        ...
                        expected:
                          (entry.expected &&
                            payloadLabels[`${entry.field}.expected`]) ||
                          (entry.type === "equalField" &&
                            payloadLabels[entry.expected]) ||
                          entry.expected,
                ...
                const err = {
                  global:
                    ...
                    typeof data.error.error === "string"
                      ? f({
                          id: `error.${data.error.error}.description`,
                          defaultMessage:
              ...
              this.setState((prev) => ({ ...prev, appState }))
              console.error(
                "whole application state response received from XHR, this is unexpected behavior but commit update",
                data,
              )
              return appState
