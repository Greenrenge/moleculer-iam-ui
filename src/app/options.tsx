...
import { ParsedLocale } from "@greenrenge/moleculer-iam-core"
...
export const AppOptionsContext = createContext<
  [AppOptions, AppOptionsProvider["setState"]]
>([] as any)
...
export class AppOptionsProvider extends React.Component<
  { initialOptions: Partial<AppOptions> },
  AppOptions
> {
  ...
  state = _.defaultsDeep(
    ...
    AppOptionsProvider.defaultOptions,
  )
  ...
  constructor(props: any) {
    ...
    const theme =
      ...
      (
        document.cookie
          .split("; ")
          .find((s) => s.trim().startsWith("theme=")) || ""
      ).split("=")[1] // cookie second
    if (
      theme &&
      theme !== this.state.theme &&
      Object.keys(this.state.palette).includes(theme)
    ) {
  ...
  render() {
    ...
    //TODO: green change this
    const locale = `${this.state.locale.language || "ko"}-${
      this.state.locale.region || "KR"
    }`
