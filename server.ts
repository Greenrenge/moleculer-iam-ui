...
} from "@greenrenge/moleculer-iam-core"
...
class SinglePageApplicationRenderer implements ApplicationRenderer {
  constructor(
    private readonly props: ApplicationRendererProps,
    private readonly options: RecursivePartial<ApplicationOptions> = {},
  ) {}
  ...
  private loadViews() {
    ...
    const options = `<script>window.__APP_DEV__=${JSON.stringify(
      this.props.dev,
    )};window.__APP_PREFIX__=${JSON.stringify(
      this.props.prefix,
    )};window.__APP_OPTIONS__=${JSON.stringify(this.options)};</script>`
  ...
  public readonly render: ApplicationRenderer["render"] = async (
    ctx,
    state,
  ) => {
    ...
    try {
      ...
      serializedState = JSON.stringify({
        error: { error: error.name, error_description: error.message },
      })
    }
  ...
  public readonly routes: ApplicationRenderer["routes"] = () => {
    return [
      ...
      serveStatic(serverConfig.webpack.output.path, {
        prefix: serverConfig.webpack.output.publicPath, // /op/assets
